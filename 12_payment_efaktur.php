...
if (isset($_GET['ins'])) {   
   ...
   $tglbayarefaktur    = $_GET['tglbayarefaktur'];  
   $iduserefaktur = $iduser;    
   $issukses = 1;
    
		
   $sql = "update tinvoice set isbayarefaktur=1, tglbayarefaktur='$tglbayarefaktur', iduserefaktur='$iduserefaktur'  "; 
   ...
   try { 
	   ...
	   $qins->execute();  
	   $pesan = "<font color=blue>Record invoice<strong>".$noinvoice." </strong> updated payment efaktur successfully</font>"; 
	}//try
...
if (isset($_GET['del'])) {
   $noinvoice = trim($_GET['noinvoice']);  
   $istidakbolehdel = 0;	  
   $sqlcek = "select * from tinvoice WHERE noinvoice='$noinvoice' and isbayarefaktur=0 "; 
    try { 
	...
	if ($istidakbolehdel == 0){
	   $sqld = "update tinvoice set isbayarefaktur=0, tglbayarefaktur='2000-01-01' WHERE noinvoice='$noinvoice'  "; 
	   try { 
...
<div class="row"> 
    <ol class="breadcrumb">
	  <li><i class="fa fa-home"></i>PAYMENT EFAKTUR</li> 
	</ol> 
	<section class="panel">
	  <header class="panel-heading">
		  ...
		  <form role="form"  method="GET" onSubmit="return validasi_input(this)"  action="index.php"> 
			 ...
			 <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6">
				  <label>Tanggal Bayar Efaktur</label>	 
				  <input name="tglbayarefaktur" id="dp1"   size="16" class="form-control" value="<?php echo $tglini; ?>" onKeyPress="if (event.keyCode==13) {nominal.focus()}" >  
			 </div>	  
			 <br>  
	 
			  <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6"> 
				 <input type="hidden" name="par" id="par" value="12">
	  ...
	  <div align="center">EFAKTUR PAYMENT <!--BULAN INI : <?= $bulan?>-<?= $tahun?>--> 
	  </div>
	  <div class="box-body">
	   <table id="contoh" class="table table-bordered table-striped table-hover"> 
		  ...
		  <?php 
			   ...
			   try { 
			        //$sql = $conn->prepare("select * from tinvoice where month(tglbayarefaktur)=$bulan and year(tglbayarefaktur)=$tahun order by tglbayarefaktur"); 
					$sql = $conn->prepare("select * from tinvoice  order by tglbayarefaktur"); 
					...
					while($rs = $sql->fetch()) { 
					    $tglbayar = '';
						if ($rs['tglbayarefaktur'] != '2000-01-01'){
							$tglbayar=$rs['tglbayarefaktur'];
						}	
					   echo "   <tr>
