...
if (isset($_GET['ins'])) {   
   ...
   $tglbayarinvoice    = $_GET['tglbayarinvoice']; 
   $bsubayarinvoice  = trim($_GET['bsubayarinvoice']); 
   $ketbayarinvoice  = trim($_GET['ketbayarinvoice']); 
   $iduserbayarinvoice = $iduser;  
   
  // echo $noinvoice .' | '.$tglbayarinvoice.' | '.$bsubayarinvoice.' | '.$ketbayarinvoice.' | '.$iduser;
   
   
   $issukses = 1;
    
		
   $sql = "update tinvoice set isbayarinvoice=1, tglbayarinvoice='$tglbayarinvoice', iduserbayarinvoice='$iduserbayarinvoice', "; 
   $sql = $sql . "bsubayarinvoice=$bsubayarinvoice, ketbayarinvoice='$ketbayarinvoice' where noinvoice='$noinvoice'"; 
 
   try { 
	   //insert
	   $qins = $conn->prepare($sql);
	   $qins->execute();  
	   $pesan = "<font color=blue>Record invoice<strong>".$noinvoice." </strong> updated payment successfully</font>"; 
	}//try
...
if (isset($_GET['del'])) {
   $noinvoice = trim($_GET['noinvoice']);	 
   $istidakbolehdel = 0;	  
   $sqlcek = "select * from tinvoice WHERE noinvoice='$noinvoice' and isbayar=0 "; 
    try { 
	   $qcek = $conn->prepare($sqlcek);
	   $qcek->execute(); 
	   if($qcek->rowCount() <1){ 
	      $istidakbolehdel = 1;
		  $pesan = "<font color=red>Invoice <strong>".$noinvoice." </strong> belum dibayarkan.</font><br>";
	...
	if ($istidakbolehdel == 0){
	   $sqld = "update tinvoice set isbayarinvoice=0, tglbayarinvoice='2000-01-01', bsubayarinvoice=0, ketbayarinvoice='-' WHERE noinvoice='$noinvoice'  "; 
	   try { 
...
function validasi_input(form){
  ...
  if (form.bsubayarinvoice.value == ""){
    alert("Nominal masih kosong!");
    form.bsubayarinvoice.focus();
    return (false);
  }
...
<div class="row"> 
	...
	<section class="panel">
	  <header class="panel-heading">
		  ...
		  <form role="form"  method="GET" onSubmit="return validasi_input(this)"  action="index.php"> 
			 ...
			 <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6">
				  ...
				  <input name="tglbayarinvoice" id="dp1"   size="16" class="form-control" value="<?php echo $tglini; ?>" onKeyPress="if (event.keyCode==13) {nominal.focus()}" >  
			 </div>	     
			 
			 <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6">
				  <label>Nominal Rp.</label>	
				  <div class="search-box-material"><input name="bsubayarinvoice" onKeyUp="this.value = this.value.toUpperCase();" class="form-control" id="bsubayarinvoice" type="text1" step="any" autocomplete="off" placeholder="Tulis nominal hanya angka"    >
				  <div class="result1"></div>   
				  </div>
			 </div>	 
			 
			 <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6">
				  <label>Ket. Pembayaran</label>	
				  <div class="search-box-material"><input name="ketbayarinvoice" onKeyUp="this.value = this.value.toUpperCase();" class="form-control" id="ketbayarinvoice" type="text1" step="any" autocomplete="off" placeholder="Keterangan Invoice"    >
				  <div class="result1"></div>   
				  </div>
			 </div>	
			 <br>  
			  ...
			  <div class="form-group col-xs-12 col-sm-6 col-md-6 col-lg-6"> 
				 <input type="hidden" name="par" id="par" value="10">
	  ...
	  <div align="center">INVOICE PAYMENT <!--BULAN INI : <?= $bulan?>-<?= $tahun?> -->
	  </div>
	  <div class="box-body">
	   <table id="contoh" class="table table-bordered table-striped table-hover"> 
		  ...
		  <?php 
			   ...
			   try { 
			        //$sql = $conn->prepare("select * from tinvoice where month(tglbayarinvoice)=$bulan and year(tglbayarinvoice)=$tahun order by tglbayarinvoice"); 
					$sql = $conn->prepare("select * from tinvoice   order by tglbayarinvoice"); 
					...
					while($rs = $sql->fetch()) { 
					   ...
					   echo "  <tr>
						  ...
						  <td>".strtoupper($rs['iduserinvoice'])."</td>
						  ...
						  <td>".$rs['isiinvoice']."</td> 
						  <td>".$tglbayar."</td>
						  <td>".$rs['bsubayarinvoice']."</td>
						  <td>".$rs['ketbayarinvoice']."</td>
						  <td>"?>
